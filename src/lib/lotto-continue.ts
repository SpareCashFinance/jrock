...
import { vrfSeedBytes } from "./lotto-vrf";
...
/**
 * After a winner is paid (or a zero-slip book is closed) the next round must
 * start in the same transaction. claim / empty close bump current_round, so
 * open_round targets currentRound + 1.
 */
export function finishFlowIxs(input: FinishFlowInput): TransactionInstruction[] {
  ...
  for (const part of finishFlowPlan(step.kind, totalTickets)) {
...
export async function deskCrankIxs(
  ...
  if (input.step.kind === "request_vrf") {
  ...
  if (input.step.kind === "store_vrf") {
    if (!input.vrfRequest) throw new Error("No VRF request is stored on this round yet.");
    return [fulfillRandomnessIxV2(input.currentRound, new PublicKey(input.vrfRequest))];
  }
  return finishFlowIxs(input);
}
